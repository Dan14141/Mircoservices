    ...
    def __call__(self, request):
        ...
        if auth_header and auth_header.startswith('Bearer'):
            ...
            if user_data:
                ...
                logger.info(f"Authenticated {user_data['id']} for {request.path}")
                ...
                return JsonResponse({'error': 'Invalid token',
                                    'message': 'The provided authentication token is invalid'}, status=401)
        else:
            logger.warning(f"No auth token in request to {request.path}")
            return JsonResponse({
                'error': 'Authentication token required',
            'message': 'Authentication header with Bearer token is required'}, status=401)

        # Если вдруг до сюдова дойдёт
        response = self.get_response(request)
        return response